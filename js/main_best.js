(() => {
  ...
  function getBestBookIsbn(isbnString = "") {
  ...
  async function initMainBest() {
    if (bestInitialized) return;
    bestInitialized = true;

    try {
      renderBestBooks("bestBookList", BEST_BOOKS.map(createBestFallbackBook));
  ...
  async function fetchSingleBestBook(item) {
    ...
    try {
      ...
      return {
        ...
        isbn: getBestBookIsbn(bestMatch.isbn),
      };
    } catch (error) {
  ...
  function createBestBookItem(item) {
    ...
    return `
      <li class="main_best_item">
        <a href="${detailLink}" class="main_best_link">
          <div class="main_best_thumb">
            <img src="${item.image}" alt="${item.title}" loading="lazy" onerror="this.src='${BEST_NO_IMAGE_PATH}'">
  ...
  window.initMainBest = initMainBest;
})();
