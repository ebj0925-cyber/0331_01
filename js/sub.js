...
/* =========================
   과제용 서버 전송 주소
   로컬 테스트 예시: json-server --watch db.json --port 3000
   db.json 예시: { "orders": [] }
========================= */
...
async function sendOrderData(book, actionType = "cart") {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("주문 데이터를 서버로 전송하지 못했습니다.");
    }

    const result = await response.json();
    console.log("전송 성공:", result);
    alert(`${actionType === "buy" ? "바로구매" : "장바구니"} 데이터가 서버로 전송되었습니다.`);
  } catch (error) {
    console.error("전송 오류:", error);
    alert("서버 전송 중 오류가 발생했습니다. ORDER_API_URL 주소를 확인해주세요.");
  }
}
